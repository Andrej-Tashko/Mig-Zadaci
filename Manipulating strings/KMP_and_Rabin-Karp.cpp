...
public:
    ...
    vector<int> rabinKarp(const String &subStr) {
        ...
        vector<long long> hash(SS + 1, 0);
        for (int i = 0; i < S; i++) {
            hash[i + 1] = (hash[i] + (subStr.chars[i] - 'a' + 1) * posPow[i]) % mod;
        }
        
        long long hashes = 0;
        for (int i = 0; i < S; i++) 
            hashes = (hashes + (chars[i] - 'a' + 1) * posPow[i]) % mod; 

        vector<int> occurrences;
        int killCounter = SS;
        for (int i = 0; i + SS - 1 < S; i++) {
            long long currentHash = (hash[i + SS] + mod - hash[i]) % mod;
            if (currentHash == hashes * posPow[i] % mod) {
                for (int j = 0; j < SS; j++) {
...
int main() {
    ...
    cout << "First occurrence of " << ss << " is on position: " << firstPosition + 1 << endl;
    cout << "----------------------------------------\n";
    ...
    cout << "First occurrence of " << ss << " in reverse order is on position: " << firstReversePosition + 1 << endl;
